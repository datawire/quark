...
primitive String {
    ...
    macro int startsWith(String other) $java{($self).startsWith($other)}
                                       $py{($self).startswith($other)}
                                       $js{(($self).indexOf($other)===0)};
    macro int endsWith(String other)   $java{($self).endsWith($other)}
    ...
    macro String replace(String from, String to) $java{($self).replace(java.util.regex.Pattern.quote($from), ($to))}
