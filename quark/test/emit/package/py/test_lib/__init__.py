...
def _construct(className, args):
    ...
    if ((className) == (u"test.Test")):
        return test.Test()

    if ((className) == (u"test.test.Test")):
...
def _fields(className):
    ...
    if ((className) == (u"test.test.Test")):
        return _List([Field(Class(u"int"), u"size")])
...
def _class(cls):
    ...
    if (((cls).id) == (u"test.Test")):
        (cls).name = u"test.Test"
        (cls).parameters = _List([])
        return

    if (((cls).id) == (u"test.test.Test")):
        (cls).name = u"test.test.Test"
        (cls).parameters = _List([])
        return
