...
def _construct(className, args):
    ...
    if ((className) == (u"test1.Test")):
        return test1.Test()

    if ((className) == (u"test2.Test")):
        return test2.Test((args)[0])

    if ((className) == (u"test3.Box<String>")):
...
def _class(cls):
    ...
    if (((cls).id) == (u"test1.Test")):
        (cls).name = u"test1.Test"
        (cls).parameters = _List([])
        return

    if (((cls).id) == (u"test2.Test")):
        (cls).name = u"test2.Test"
        (cls).parameters = _List([])
        return

    if (((cls).id) == (u"test3.Box<String>")):
        (cls).name = u"test3.Box"
        (cls).parameters = _List([Class(u"String")])
        return
