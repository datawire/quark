...
def compile(comp, key, fun):
    ...
    t = comp.types.unresolve(types.resolve(comp, fun).result)
...
@cmatch(Type)
def compile(comp, t):
    return compile(comp, comp.types.unresolve(types.resolve(comp, t)))
...
def compile(comp, call):
    t = comp.types.unresolve(types.resolve(comp, call.expr))
