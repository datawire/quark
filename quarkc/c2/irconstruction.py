...
def compile(comp, key, fun):
    ...
    t = comp.types.unresolve(comp.types.resolve(fun).result)
...
@cmatch(Type)
def compile(comp, t):
    return compile(comp, comp.types.unresolve(comp.types.resolve(t)))
...
def compile(comp, call):
    t = comp.types.unresolve(comp.types.resolve(call.expr))
