...
You must have Ruby 2.3 installed to use any Ruby code generated by {{{product}}}.


Alternative Usage via Docker
----------------------------

If you don't want to install anything you can run a minimal test of your code using a Docker image.
For example, let's assume you haven't installed Java but you want to make sure your code runs with Java.
You can do so by running:

``docker run -v $PWD:/code datawire/quark-run --java path/to/your/file.q``

This is equivalent to doing ``quark install --java path/to/your/file.q`` followed by  ``quark run --java path/to/your/file.q``.
You can also use ``--javascript``, ``--ruby``, or ``--python`` instead of ``--java``.
